...
% This file is PRCV/LNCS-ready and requires the official Springer template
% files, including llncs.cls and splncs04.bst, to compile.
...
\end{equation}
Following the variable-token view in multivariate time-series forecasting~\cite{liu2023itransformer,liu2025timecma}, each stock is treated as a variable or node. This is crucial in our setting because the co-occurrence graph and the time-series branch share the same node semantics.

\subsection{Text-Induced Stock Co-Occurrence Graph}

Following text-induced relation modeling~\cite{li2025cograsp}, let $D = \{d_1, \dots, d_M\}$ be the document set, and let $S_m \subseteq S$ be the set of stocks mentioned in document $d_m$. The static co-occurrence count between stocks $s_i$ and $s_j$ is:
\begin{equation}
...
\end{equation}
We use this lightweight propagation operator rather than a full graph neural network to isolate the effect of text-induced relation priors and reduce additional modeling complexity~\cite{kipf2017semi,velivckovic2018graph}.
...
Cross-modal alignment is implemented with scaled dot-product attention, following the TimeCMA alignment principle~\cite{liu2025timecma}. Specifically, the graph-enhanced time-series representation is used as query, and the graph-enhanced prompt representation is used as key and value:
\begin{equation}
...
\end{equation}
Although the task is trained as regression, evaluation emphasizes finance-oriented ranking metrics. We report MSE, MAE, IC, RankIC, ICIR, and RankICIR. IC and RankIC measure cross-sectional predictive quality, while ICIR and RankICIR further characterize stability across rolling windows~\cite{grinold2000active}.
...
\textbf{Conventional sequence models} include MLP, LSTM~\cite{hochreiter1997long}, ALSTM, Transformer~\cite{vaswani2017attention}, and XGBoost~\cite{chen2016xgboost}.
