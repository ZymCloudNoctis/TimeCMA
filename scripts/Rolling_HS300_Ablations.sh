...
echo "开始运行 HS300 第一批消融实验..."
echo "行情数据: $market_data"
echo "雪球数据: $snowball_csv"
echo "方法: no_graph static dynamic6m dynamic6m_no_decay static_no_graph_preproc static_no_winsorize"
echo "测试月份: 2025-04 ~ 2025-09"
echo "默认图权重变换: log1p + top-k(10)"
echo "动态图时间衰减: exp, half-life=60 days"
echo "默认训练标签 winsorize: 1% ~ 99%"

"$python_bin" -u run_rolling_experiments.py \
  --market_data "$market_data" \
  --snowball_csv "$snowball_csv" \
  --methods no_graph static dynamic6m dynamic6m_no_decay static_no_graph_preproc static_no_winsorize \
