...
results_dir = './results/600519_daily/'
...
class trainer:
    def __init__(
        ...
        self.loss = torch.nn.SmoothL1Loss() # 引入 Huber 损失，对离群点/极端波动更鲁棒
        self.MAE = MAE
        self.clip = 5
...
def load_data(args):
    ...
    train_set = data_class(root_path=args.root_path, flag='train', scale=True, size=[args.seq_len, 0, args.pred_len], data_path=args.data_path, freq=args.freq, num_nodes=args.num_nodes)
    val_set = data_class(root_path=args.root_path, flag='val', scale=True, size=[args.seq_len, 0, args.pred_len], data_path=args.data_path, freq=args.freq, num_nodes=args.num_nodes)
    test_set = data_class(root_path=args.root_path, flag='test', scale=True, size=[args.seq_len, 0, args.pred_len], data_path=args.data_path, freq=args.freq, num_nodes=args.num_nodes)
...
def main():
    ...
    # 保存预测结果用于绘图
    res_path = os.path.join("./results/", args.data_path)
    os.makedirs(res_path, exist_ok=True)
    np.save(os.path.join(res_path, 'preds.npy'), test_pre.cpu().numpy())
    np.save(os.path.join(res_path, 'trues.npy'), test_real.cpu().numpy())
